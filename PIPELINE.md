...
| SR15 | Performance budget. **Instrumentation + permissive CI assertion shipped** — `lib/searchMetrics.ts` tracks keystroke→paint via `performance.mark` + `performance.measure`, PerformanceObserver aggregates into a rolling 100-sample buffer, stats exposed on `window.__cd_search_perf`. `tests/e2e/search-perf.spec.ts` exercises the path + asserts `p95 < 800 ms` (permissive ceiling — local Mac runs ~390-420 ms; first few CI runs publish baseline numbers via `console.log`). **Remaining**: (a) tighten the ceiling toward the spec's 200 ms (likely requires dropping the 200 ms debounce to ~50 ms first — Notion-style); (b) type-ahead < 80 ms once SR10 lands; (c) OpenSearch round-trip < 150 ms once S2 lands. | [`12-search-surface`](./docs/ux/12-search-surface.md) §"Performance budget" | P1 | Tighten threshold after 5+ CI runs of baseline; SR10 / S2 unblock the type-ahead + opensearch bullets |
