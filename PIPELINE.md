...
| # | Item | Brief | Priority | Trigger |
|---|---|---|---|---|
| MU1 | Workspace invitations. **Phases 1a + 1b shipped.** 1a: migration 0013 + `WorkspaceInvitationRepo` + 5 endpoints (create / list / revoke / peek / accept) + 8 integration tests. 1b: SPA UI — `InviteDialog` Radix modal (role + expiry + reuse radio, single-use vs multi-use), switcher-footer "Invite to <Workspace>" entry for team workspaces (hidden on personal + DEMO_MODE), `/invite/<token>` `InviteAccept` page rendering BEFORE the auth gate so anonymous peek works; signed-in visitors get "Join workspace" → POST accept → switch active workspace + route home; anonymous visitors get "Sign in to join" → bounce. **Remaining**: 1c — Settings → Members tab (active + pending list with revoke buttons); 1d — magic-link auto-create (anonymous → mint user + session). | [[workspace-invitations]] memory + (TODO) `docs/research/18-workspace-invitations.md` | P1 | 1c adds the management surface; 1d ships the locked "magic-link auto-create" path |
