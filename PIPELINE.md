...
| SR15 | Performance budget. **Instrumentation shipped** — `lib/searchMetrics.ts` tracks keystroke→paint via `performance.mark` + `performance.measure`, PerformanceObserver aggregates into a rolling 100-sample buffer, stats exposed on `window.__cd_search_perf` for Playwright + DevTools. Remaining: (a) Playwright e2e that asserts p95 < 200 ms against the demo backend; (b) type-ahead < 80 ms once SR10 lands; (c) OpenSearch round-trip < 150 ms once S2 lands. | [`12-search-surface`](./docs/ux/12-search-surface.md) §"Performance budget" | P1 | CI assertion follows after a few runs of baseline numbers |
