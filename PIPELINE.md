...
| ED4 | **Excalidraw editor for `.excalidraw` files** — third co-edit format alongside `.docx` / `.xlsx`. End-to-end shape locked 2026-06-10: <br/>• **Create**: Sidebar "New" menu gains "New diagram" alongside Document / Spreadsheet. <br/>• **Editor chrome**: Preview-modal-by-default with same AutosaveStatus + Cmd-S as docx/xlsx; a corner "Open fullscreen" button promotes to a `/d/<file_id>` route for room-to-work. <br/>• **Storage**: native JSON via existing `GET / PUT /api/files/{id}/content`. New MIME `application/vnd.excalidraw+json`; `inferKind` gains `excalidraw` bucket. <br/>• **Thumbnails**: PNG via the existing sandboxed `drive-thumb-worker` (same threat model as video frames). Regenerated every save; share-link preview uses the same PNG. <br/>• **Co-edit**: opt-in via `VITE_DRIVE_COLLAB_BACKEND_URL=wss://...` matching the docx/xlsx pattern. Single-user when unset. Collab server lives in a sibling repo (TBD path); not bundled in the Drive binary. <br/>• **Theme**: passes `prefers-color-scheme` through Excalidraw's `theme` prop. Saved JSON stays theme-agnostic. <br/>• **Bundle**: ~1.5 MB, `React.lazy()` so it only ships on file open. <br/>Brief: `docs/research/19-excalidraw.md` (TODO). | — (needs brief — UX decisions in [[excalidraw-integration]]) | P2 | User asked 2026-06-10; ready to schedule once a sprint window opens. Sibling collab-server repo decision blocks the co-edit half. |
