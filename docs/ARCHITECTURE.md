...
- `GET /metrics` — **Prometheus** exposition: HTTP responses by status class, in-flight gauge, uptime, a request-latency histogram (`dochub_http_request_duration_seconds`, 5ms–10s buckets + `_sum`/`_count`) for p95/p99 SLOs, and background-job SLIs (`dochub_jobs{state=…}` queue depth + `dochub_jobs_oldest_queued_age_seconds` processing lag, read from the DB at scrape time). Fed by the access-log middleware, so it reflects real served traffic. Only non-sensitive aggregates; restrict by network policy if needed.
