...
| Search by tag | **Shipped** | `document_tags` migration (`0024_tags.sql`); tag CRUD + assign/unassign routes (`tags_http`); `GET /api/search?tag=…` AND-filters (a file must carry every listed tag); tag UI wired | Facet counts / tag-cloud UX (polish) | — |
...
| Frontend state (Redux etc.) | **In progress** | Redux Toolkit + RTK Query foundation landed (`web/src/store`): typed API client over `client.ts` (reuses CSRF/credentials/demo/errors), store + `<Provider>`, **session slice** (`me`/`setupStatus`) migrated — AuthContext now sources session state from the RTK cache. Error boundaries already shipped (app + editor surfaces). | Migrate remaining domains slice-by-slice (workspaces, files, tags, search) off Context/rxjs onto RTK Query hooks | M |
