...
/// load `DOCHUB_STORAGE_SECRET_KEY`. Anything other than exactly 32 bytes
...
pub fn parse_master_key_hex(hex: &str) -> Result<[u8; 32], &'static str> {
    ...
    if trimmed.len() != 64 {
        return Err("DOCHUB_STORAGE_SECRET_KEY must be 64 hex chars (32 bytes)");
    }
    let mut out = [0u8; 32];
    for (i, chunk) in trimmed.as_bytes().chunks(2).enumerate() {
        let hi = hex_digit(chunk[0]).ok_or("DOCHUB_STORAGE_SECRET_KEY contains non-hex")?;
        let lo = hex_digit(chunk[1]).ok_or("DOCHUB_STORAGE_SECRET_KEY contains non-hex")?;
        out[i] = (hi << 4) | lo;
    }
    Ok(out)
}
