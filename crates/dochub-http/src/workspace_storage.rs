            ...
            Self::KeyMissing => (
                ...
                Json(Err {
                    error: "server has no storage secret key configured; \
                         set DOCHUB_STORAGE_SECRET_KEY (64 hex chars) and restart",
                }),
            )
                .into_response(),
            Self::Validation(m) => {
...
fn allow_insecure() -> bool {
    matches!(
        std::env::var("DOCHUB_ALLOW_INSECURE_BYO").ok().as_deref(),
        Some("1" | "true" | "yes")
    )
}
...
async fn test_storage(
    ...
    let target_name = require_owner(&s, &session, &id).await?;
    let cfg = ByoConfig {
        provider: provider_to_dochub_storage(body.provider),
        bucket: body.bucket,
        region: body.region,
...
async fn patch_credentials(
    ...
    let cfg = ByoConfig {
        provider: provider_to_dochub_storage(existing.provider),
...
fn provider_to_dochub_storage(p: WorkspaceStorageProvider) -> dochub_storage::Provider {
    match p {
        WorkspaceStorageProvider::S3 => dochub_storage::Provider::S3,
        WorkspaceStorageProvider::Minio => dochub_storage::Provider::Minio,
        WorkspaceStorageProvider::R2 => dochub_storage::Provider::R2,
        WorkspaceStorageProvider::B2 => dochub_storage::Provider::B2,
    }
}
...
pub(crate) async fn resolve_upload_storage(
    s: &HttpState,
    workspace_id: &str,
) -> Result<(std::sync::Arc<dochub_storage::Storage>, Option<String>), crate::files::FilesError> {
    let row = WorkspaceStorageRepo::new(&s.db)
    ...
    let key = s.storage_secret_key.as_ref().ok_or_else(|| {
        crate::files::FilesError::Internal(
            "workspace storage configured but DOCHUB_STORAGE_SECRET_KEY is missing".into(),
        )
    })?;
    let secret_bytes =
        dochub_storage::open_secret(key, &row.secret_ct, &row.aad()).map_err(|e| {
    ...
    let cfg = ByoConfig {
        provider: provider_to_dochub_storage(row.provider),
