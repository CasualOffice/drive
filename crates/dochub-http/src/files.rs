...
impl EditorTarget {
    ...
    fn origin<'a>(&self, cfg: &'a dochub_core::Config) -> Option<&'a url::Url> {
        match self {
...
async fn open_in_editor(
    ...
    let file_id_typed = dochub_core::FileId::from_str(&file.id)
        .map_err(|e| FilesError::Internal(format!("file id parse: {e}")))?;
    let perms = dochub_wopi::WopiPerms::Write;
    let exp = time::OffsetDateTime::now_utc().unix_timestamp() + HANDOFF_TTL_SECS;
    let claims = dochub_wopi::WopiClaims {
    ...
    let access_token = dochub_wopi::mint_token(&s.jwt_secret, &claims);
...
/// `crates/dochub-storage`, bumps `size` + `version` + `modified_at`
