...
use dochub_db::{AuditRepo, NewAuditEvent, NewSession, SessionRepo};
...
pub(crate) async fn callback(
    ...
    let sid = dochub_auth::generate_session_id();
    let csrf = dochub_auth::generate_csrf_token();
...
fn build_session_cookie(sid: &str, secure: bool, ttl: time::Duration) -> String {
    let name = if secure { "__Host-dh_sid" } else { "dh_sid" };
