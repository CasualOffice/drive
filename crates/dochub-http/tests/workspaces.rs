...
use dochub_db::{Db, NewUser, UserRepo, WorkspaceMemberRepo, WorkspaceRepo, WorkspaceRole};
...
async fn transfer_ownership_swaps_roles_atomically() {
    ...
    let team = WorkspaceRepo::new(&state.db)
        .insert("Engineering", dochub_db::WorkspaceKind::Team, &admin.id)
        .await
        .unwrap();
    ...
    let app_state_db = dochub_db::Db::connect("sqlite::memory:").await.unwrap();
...
async fn list_members_returns_username_and_admin_flag() {
    ...
    let bob_id = UserRepo::new(&state.db)
        ...
        .id;
    let team = WorkspaceRepo::new(&state.db)
        .insert("Engineering", dochub_db::WorkspaceKind::Team, &admin_id)
        .await
        .unwrap()
        .id;
    WorkspaceMemberRepo::new(&state.db)
