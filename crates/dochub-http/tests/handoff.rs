...
async fn open_token_passes_wopi_verify() {
    ...
    use std::str::FromStr;
    let parsed_id = dochub_core::FileId::from_str(&fid).unwrap();
    let claims = dochub_wopi::verify_token(&jwt_secret, token, parsed_id).unwrap();
    assert_eq!(claims.file_id, parsed_id);
    assert_eq!(claims.perms, dochub_wopi::WopiPerms::Write);
}
