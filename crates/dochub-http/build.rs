...
//
//   DOCHUB_GIT_SHA   — short git SHA, or "unknown" if not in a git tree
//   DOCHUB_BUILT_AT  — RFC-3339 build timestamp (seconds precision)
//
...
fn main() {
    println!("cargo:rustc-env=DOCHUB_GIT_SHA={}", git_short_sha());
    println!("cargo:rustc-env=DOCHUB_BUILT_AT={}", built_at());
